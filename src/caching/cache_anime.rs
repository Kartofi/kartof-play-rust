...
impl Database {
    ...
    fn create_new(&self, id: &str, id_type: IdType) -> mongodb::error::Result<CacheResult> {
        ...
        if id_type == IdType::Gogoanime {
            ...
            if animegg_search.len() > 0 {
                ...
                for anime_res in animegg_search {
                    if
                        anime_res.episodes.abs_diff(anime.details.episodes) <=
                ...
                if found == true {
                    ...
                    if
                        result_anime.episodes > anime.details.episodes &&
        ...
        let schedule_search = scrapers::anime_schedule::anime_search
            ::get(&title)
            .unwrap_or_default();
        if schedule_search.len() > 0 {
            ...
            if id_schedule != "https://animeschedule.net" && id_schedule != "" {
                if result_schedule.new_ep == 0 {
                    ...
                    let schedule_data = scrapers::anime_schedule::anime_details::get(
                        &id_schedule,
                        is_dub
                    );
    ...
    fn update_existing(&self, current: &mut Anime) -> mongodb::error::Result<CacheResult> {
        ...
        if current.animegg_id.len() == 0 {
            let animegg_search = scrapers::animegg::anime_search
                ::get(&current.title)
                .unwrap_or_default();
            if animegg_search.len() > 0 {
        ...
        if current.details.episodes < details.episodes {
            ...
            let mut ep_list_animegg = scrapers::animegg::anime_details::get_episodes(
                &&current.animegg_id
            );
            ...
            if ep_list_gogo.len() >= ep_list_animegg.len() {
                let end_iter = if ep_list_gogo.len() == 1 { 1 } else { ep_list_gogo.len() - 1 };
                for i in 0..end_iter {
                    ...
                    pool.execute(move || {
                        let gogo_url = scrapers::gogoanime::anime_stream
                            ::get(&ep_gogo)
                            .unwrap_or_default();
                        let mut animegg_url = "".to_string();
                        if animegg_len > i {
                            animegg_url = scrapers::animegg::anime_stream
                                ::get(&ep_anime)
                                .unwrap_or_default();
                        }

                        let mut ep = Episode::new();
                        ep.num = ep_gogo.split("-episode-").last().unwrap_or_default().to_string();
                ...
                for i in 0..end_iter {
                    ...
                    let cl = Arc::clone(&episodes_new_mt);
                    pool.execute(move || {
                        let animegg_url = scrapers::animegg::anime_stream
                            ::get(&ep_animegg)
                            .unwrap_or_default();
                        let mut gogo_url = "".to_string();
                        if gogo_len > i {
                            gogo_url = scrapers::gogoanime::anime_stream
                                ::get(&ep_gogo)
                                .unwrap_or_default();
                        }
...
fn compare(a: &str, b: &str) -> Ordering {
    match (a.replace("-", ".").parse::<f32>(), b.replace("-", ".").parse::<f32>()) {
    ...
    for i in 0..episodes_gogo.len() {
        ...
        pool.execute(move || {
            ...
            if ep.is_none() {
                ...
                let ep_url = scrapers::gogoanime::anime_stream
                    ::get(&episodes_gogo[i])
                    .unwrap_or_default();
    ...
    for i in 0..episodes_animegg.len() {
        ...
        pool.execute(move || {
            ...
            if ep.is_none() {
                ...
                let ep_url = scrapers::animegg::anime_stream
                    ::get(&episodes_animegg[i])
                    .unwrap_or_default();
